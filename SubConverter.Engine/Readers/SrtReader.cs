...
public class SrtReader : IReader
{
    public List<InternalSubtitle> Read(FileInfo file, AdditionalInfo info)
    {
        ...
        do
        {
            block = ReadNextBlock(lines, ref linePointer);
    ...
    private static InternalSubtitle? ReadNextBlock(string[] lines, ref int linePointer)
    {
        ...
        // Read first not empty line as block index
        var indexLine = lines[linePointer++].Trim();
        if(!int.TryParse(indexLine, out int index)) throw new FormatException($"Wrong format in line {linePointer - 1}, expected block index (integer), instead got: {indexLine}");
        ...
        while(linePointer < lines.Length && !string.IsNullOrWhiteSpace(lines[linePointer]))
            sb.AppendLine(lines[linePointer++]);
